...
std::vector<std::string> wordArray = splitStringBySpace(BIP);
int analogPins[] = {1, 2, 3, 4}; 
...
void sampleChannels() {
  for (int ch = 0; ch < NUM_CHANNELS; ch++) {
    ...
    // ESP32-S3 has a 12-bit ADC (0-4095) and uses a 3.3V reference voltage.
...
void loop() {
  ...
  // IMPORTANT: The following for-loop will not execute because the BIP string
